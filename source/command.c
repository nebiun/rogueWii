...
void command()
{
	...
	while (ntimes--) {
		...
		if (no_command) {
			...
			if (count && !running)
				count--;
			if (ch != 'a' && ch != RC_KEY_ESCAPE
			    && !(running || count || to_death)) {
 ...
 over:
			...
			case RC_KEY_PICKUP:
				{
					THING *obj = NULL;
					int found = 0;
					for (obj = lvl_obj; obj != NULL;
					     obj = next(obj)) {
						if (obj->o_pos.y == hero.y
						    && obj->o_pos.x == hero.x) {
							found = 1;
							break;
						}
			...
			case RC_KEY_RIGHT:
				do_move(0, 1);
				break;
			case RC_KEY_UPANDLEFT:
				do_move(-1, -1);
				break;
			case RC_KEY_UPANDRIGHT:
				do_move(-1, 1);
				break;
			case RC_KEY_DOWNANDLEFT:
				do_move(1, -1);
				break;
			case RC_KEY_DOWNANDRIGHT:
				do_move(1, 1);
				break;
			case CHR_UP(RC_KEY_LEFT):
				do_run(RC_KEY_LEFT);
				break;
			case CHR_UP(RC_KEY_DOWN):
				do_run(RC_KEY_DOWN);
				break;
			case CHR_UP(RC_KEY_UP):
				do_run(RC_KEY_UP);
				break;
			case CHR_UP(RC_KEY_RIGHT):
				do_run(RC_KEY_RIGHT);
				break;
			case CHR_UP(RC_KEY_UPANDLEFT):
				do_run(RC_KEY_UPANDLEFT);
				break;
			case CHR_UP(RC_KEY_UPANDRIGHT):
				do_run(RC_KEY_UPANDRIGHT);
				break;
			case CHR_UP(RC_KEY_DOWNANDLEFT):
				do_run(RC_KEY_DOWNANDLEFT);
				break;
			case CHR_UP(RC_KEY_DOWNANDRIGHT):
				do_run('n');
				break;
			case CHR_CTRL(RC_KEY_LEFT):
			case CHR_CTRL(RC_KEY_DOWN):
			case CHR_CTRL(RC_KEY_UP):
			case CHR_CTRL(RC_KEY_RIGHT):
			case CHR_CTRL(RC_KEY_UPANDLEFT):
			case CHR_CTRL(RC_KEY_UPANDRIGHT):
			case CHR_CTRL(RC_KEY_DOWNANDLEFT):
			case CHR_CTRL(RC_KEY_DOWNANDRIGHT):
				{
			...
			case RC_KEY_QUIT:
				...
				q_comm = FALSE;
				break;
			case RC_KEY_INVENTORY:
				after = FALSE;
				inventory(pack, 0);
				break;
			case RC_KEY_SELECTIVEINVENTORY:
				after = FALSE;
				picky_inven();
				break;
			case RC_KEY_DROP:
			...
			case RC_KEY_SAVE:
				...
				save_game();
				break;
			case RC_KEY_REST:	/* Rest command */
				break;
			case RC_KEY_CONTINUE:
			...
			case RC_KEY_MOVE:
				...
				break;
			case RC_KEY_SHOWWEAPON:
				current(cur_weapon, "wielding", NULL);
				break;
			case RC_KEY_SHOWARMOR:
				current(cur_armor, "wearing", NULL);
				break;
			case RC_KEY_SHOWRING:
...
void help()
{
	...
	for (strp = helpstr; strp->h_desc != NULL; strp++)
		if (strp->h_print) {
			wmove(hw, cnt % numprint,
			      cnt >= numprint ? COLS / 2 : 0);
			if (strp->h_ch)
	...
	wmove(hw, LINES - 1, 0);
	waddstr(hw, RS_PRESS_KEY_TO_CONTINUE2);
	wrefresh(hw);
